...
let ticking = false; // Used to throttle the scroll event
    ...
    if (!ticking) {
        window.requestAnimationFrame(function() {
            ...
            if (currentScroll > lastScrollTop && currentScroll > 80) { // Hide navbar after 80px of scroll
                document.getElementById("navbar").style.transform = "translateY(-100%)"; // Fully hide navbar
            } else {
                document.getElementById("navbar").style.transform = "translateY(0)"; // Show navbar
            }
        ...
        ticking = true; // Set the ticking flag to prevent multiple requests
    }
});
